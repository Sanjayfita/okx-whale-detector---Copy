export type StrategyCandidateMetricDirection = 'HIGHER_IS_BETTER' | 'LOWER_IS_BETTER';
...
export const compareStrategyCandidates = (input: {
  ...
  if (input.baselineCandidateId === input.candidateCandidateId) {
    throw new Error('Candidate comparison requires two different candidate IDs');
  }
  if (input.metrics.length === 0) throw new Error('metrics must contain at least one metric');

  const names = new Set<string>();
  const metrics = input.metrics
    .map((metric): StrategyCandidateMetricComparison => {
      if (metric.name.trim() === '') throw new Error('Metric name must be non-empty');
      if (names.has(metric.name)) throw new Error(`Duplicate metric name: ${metric.name}`);
      names.add(metric.name);
      const weight = metric.weight ?? 1;
      if (!Number.isFinite(weight) || weight <= 0) throw new Error('Metric weight must be positive');
      if (metric.baseline === null || metric.candidate === null) {
        return Object.freeze({ ...metric, weight: undefined, delta: null, weightedScore: null, outcome: 'UNAVAILABLE' });
      }
      ...
      const directionalDelta = metric.direction === 'HIGHER_IS_BETTER' ? delta : -delta;
      return Object.freeze({
        ...
        outcome: directionalDelta > 0 ? 'IMPROVED' : directionalDelta < 0 ? 'DEGRADED' : 'UNCHANGED',
      });
    })
    .sort((left, right) => left.name.localeCompare(right.name));

  const improvedCount = metrics.filter((metric) => metric.outcome === 'IMPROVED').length;
  const degradedCount = metrics.filter((metric) => metric.outcome === 'DEGRADED').length;
  const unchangedCount = metrics.filter((metric) => metric.outcome === 'UNCHANGED').length;
  const unavailableCount = metrics.filter((metric) => metric.outcome === 'UNAVAILABLE').length;
  const available = metrics.filter((metric) => metric.weightedScore !== null);
  const totalWeightedScore = available.reduce((sum, metric) => sum + (metric.weightedScore ?? 0), 0);
  const verdict =
    available.length === 0
      ? 'INSUFFICIENT_DATA'
