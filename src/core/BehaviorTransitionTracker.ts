...
export class BehaviorTransitionTracker {
  ...
  public getEnteredBehaviors(
    whale: Whale,
    currentBehaviors: WhaleBehavior[],
  ): WhaleBehavior[] {
    const whaleKey = this.getWhaleKey(whale);

    const previousTypes =
      this.activeBehaviors.get(whaleKey) ?? new Set<WhaleBehaviorType>();

    const currentTypes = new Set(
      currentBehaviors.map((behavior) => behavior.type),
    );

    const enteredBehaviors = currentBehaviors.filter(
      (behavior) => !previousTypes.has(behavior.type),
    );

    if (currentTypes.size > 0) {
      this.activeBehaviors.set(whaleKey, currentTypes);
    } else {
