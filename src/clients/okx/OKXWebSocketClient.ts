...
export class OKXWebSocketClient {
  ...
  private readonly orderBookSubscriptions = new Map<string, string>();
  ...
  private handleMessage(data: WebSocket.RawData): void {
    ...
    if (channel === 'books') {
      ...
      try {
        ...
        console.error(
          `Order-book callback failed for ` + `${update.instId}:`,
          error,
        );
      }
    ...
    if (channel.startsWith('candle')) {
      const rawCandle = message.data[0];
      ...
      try {
        ...
        console.error(
          `Candle callback failed for ` + `${update.instId}:`,
          error,
        );
      }
    }
  ...
  private sendOrderBookSubscription(instId: string, instType: string): void {
    if (this.ws?.readyState !== WebSocket.OPEN) {
      return;
    }
  ...
  public subscribeToOrderBook(instId: string, instType: string = 'SPOT'): void {
    if (instType !== 'SPOT') {
      throw new Error(
